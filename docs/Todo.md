...
- [X] 세미나에서 뼈대로 남겨둔 Read(전체) / Read(단건) / Update / Delete 를 main에 맞게 직접 완성해주세요.


- [X] 존재하지 않는 ID로 조회/수정/삭제 시도 시 PostNotFoundException 커스텀 예외를 만들어 던져주세요


- [X] 클래스 간 책임 분리를 명확하게 해주세요. PostService 안에 섞여있는 유효성 검증 로직을 별도 클래스로 분리해보세요


- [X] Main을 클라이언트라고 가정했는데요. 클라이언트(Main)가 서버(Controller)로부터 받는 응답을 일관된 형식으로 처리할 수 있도록, 공통 응답 객체를 설계하고 적용해보세요.



## 1주차 코드 리뷰
1. 예외 처리 및 응답 구조 개선
- [X] 에러 관리 체계 구축: ErrorCode Enum을 신설하여 에러 코드(예: INVALID_POST_ID)와 기본 메시지를 통합 관리하기


- [ ] 메서드 오버로딩 활용: 상황에 따라 사용자 정의 메시지를 직접 전달할 수 있도록 응답 메서드 확장하기


- [X] Global Exception Handler 적용: ExceptionHandler의 반환 타입을 ApiResponse<?>로 통일하여 다양한 응답 DTO를 하나의 공통 객체로 처리하기


- [X] 예외 메시지 전달: PostNotFoundException 발생 시 e.getMessage()를 활용하여 실패 응답 반환하기
...
3. DTO 및 네이밍 최적화
- [X] 응답 DTO 분리: 현재 공통으로 사용 중인 PostResponse를 각 API 기능에 맞춰 분리하기 (향후 유지보수 시 사이드 이펙트 방지)
...
- [X] 커스텀 예외 구조 개선: BaseException을 정의하고 모든 커스텀 예외가 이를 상속받도록 설계하여 GEH 관리 효율화
...
- [X] 에러 메시지 관리 체계화: PostNotFoundException의 메시지 포맷(POST_001 등)을 별도 상수나 Enum으로 분리 관리
